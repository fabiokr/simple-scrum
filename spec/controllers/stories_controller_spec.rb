...
describe StoriesController do
  ...
  it "should require user" do
    ...
    get :show, :product_id => @product.slug, :id => @story.id
    response.should redirect_to(new_session_path)

    get :new, :product_id => @product.slug
    response.should redirect_to(new_session_path)

    get :edit, :product_id => @product.slug, :id => @story.id
    response.should redirect_to(new_session_path)

    story = Factory.build(:story)
    post :create, :product_id => @product.slug, :story => story.attributes
    response.should redirect_to(new_session_path)

    @story.name = 'new name'
    post :update, :product_id => @product.slug, :id => @story.id, :story => @story.attributes
    response.should redirect_to(new_session_path)

    get :destroy, :product_id => @product.slug, :id => @story.id
    response.should redirect_to(new_session_path)
  end
  ...
  it "should user stamp the story on create and update" do
    ...
    post :update, :product_id => @product.slug, :id => story.id, :story => {:name => 'new name'}
    assigns(:story).creator_id.should == user.id
    assigns(:story).updater_id.should == @user.id
  end
  ...
  it "should add new story on :create" do
    ...
    response.should redirect_to(edit_product_story_path(@product.slug, assigns(:story)))
  ...
  it "should save story on :update" do
    ...
    response.should redirect_to(edit_product_story_path(@product.slug, @story))
    flash[:message].should_not be_nil
  end
  ...
  it "should delete story on :destroy" do
    ...
    response.should redirect_to(product_stories_path(@product.slug))
    flash[:message].should_not be_nil
  end

  it "should delete story on :destroy and set header to ok if xhr" do
    xhr 'get', :destroy, :product_id => @product.slug, :id => @story.id
